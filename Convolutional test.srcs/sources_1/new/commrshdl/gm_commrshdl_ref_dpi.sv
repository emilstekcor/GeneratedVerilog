...
byte unsigned  dpi_4_temp;

    import "DPI-C" function chandle DPI_gm_commrshdl_ref_initialize(chandle existhandle);
    import "DPI-C" function void DPI_gm_commrshdl_ref_output(input chandle objhandle,
/*Simulink signal name: 'dpi_1'*/
output byte dpi_1,
/*Simulink signal name: 'dpi_2'*/
output byte unsigned dpi_2,
...
/*Simulink signal name: 'dpi_4'*/
output byte unsigned dpi_4);
    import "DPI-C" function void DPI_gm_commrshdl_ref_update(input chandle objhandle);
    

	always @(reset) begin
        objhandle = DPI_gm_commrshdl_ref_initialize(objhandle);
        ...
        if(clk_enable == 1'b1) begin
            if(overclocking_counter==1) begin
                for(int ii=0; ii < loop_factor; ii=ii+1) begin       
                    DPI_gm_commrshdl_ref_output(objhandle,
dpi_1_temp, dpi_2_temp, dpi_3_temp, dpi_4_temp);
                    DPI_gm_commrshdl_ref_update(objhandle);   
                end
                dpi_1 <=dpi_1_temp;
dpi_2 <=dpi_2_temp;
dpi_3 <=dpi_3_temp;
