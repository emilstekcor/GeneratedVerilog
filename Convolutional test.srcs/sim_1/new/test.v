...
module tb_LTE_Convolutional_System;
  ...
  initial begin
    ...
    // Reset the system
    #10 reset_x = 1;  // Deassert reset
    #20 reset_x = 0;
    #20 enb = 1;  // Enable system operation

    // Send 1000 packets of random data
    ctrlIn_valid = 1;
    for (i = 0; i < 1000; i = i + 1) begin
        if (i == 0)
            ctrlIn_start = 1; // Signal start of the first packet
        else
            ctrlIn_start = 0;
        
        dataIn = $random % 16 - 8;  // Generate random data in the range -8 to 7
        #10;  // Maintain input for one clock cycle
    end
    ctrlIn_end = 1; // Signal end of the last packet
    #10;
    ctrlIn_end = 0;
    ctrlIn_valid = 0;
    #20;
    enb = 1;  // Disable system after sending data
